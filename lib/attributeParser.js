...
AttributeContext.prototype.replaceMatches = function(content) {
  ...
  this.matches.forEach(function(match) {
    ...
    if (match.containsTemplate) {
            // If root is not a url - it's not possible to parse it
            if (self.root !== undefined && self.parseDynamicRoutes && loaderUtils.isUrlRequest(self.root, '.')) {
                // If path does not start with a template and is not an url - it's not acceptable
                if (!match.value.match(/^\{\{/) && loaderUtils.isUrlRequest(match.value, self.root)) {
                  throw new Error('Dynamic route "' + match.value + '" cannot be required')
                }
...
module.exports = function parse(html, isRelevantTagAttr, usid, root, parseDynamicRoute) {
  var context = new AttributeContext(isRelevantTagAttr, usid, root, parseDynamicRoute);
